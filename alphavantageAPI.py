...
def getDataIntraDay(symbol, interval, datatype, api_key):
    data = { "function": "TIME_SERIES_INTRADAY",
             ...
             "datatype": datatype,
             "apikey": api_key}
    url_response = requests.get(url, params=data).content
...
def createFrame(data):
    ...
    csv_data = data.text
    writeCSV(data, "csv_data.csv")
    df = pd.read_csv("csv_data.csv")
    print(df)


# TODO: refactor "fieldname" input for dynamic input_data; separate bottom statement into a readCSV function
# We don't need this to transform API requests to pandas dataframes but we can use it later when we need to write csv's
def writeCSV(input_data, out_file):
...
# TODO: create a separate main.py file for better organization
#---TEST METHOD---#
dailyAAPL = getDataDaily("AAPL", "csv", api_key)
print(dailyAAPL)
# printData(dailyAAPL)
# createFrame(dailyAAPL)
